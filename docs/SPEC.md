...
| 4     | No clock times on screen — windows only. "I prayed Dhuhr" drives the sunnah. **Bent 2026-09-20**: Today may say a rough distance to the next prayer ("in about an hour"), never a time                                         | Removes the prayer-time accuracy trap; matches real behaviour. A duration says "there is time for the siwak" without promising a minute                                                      |
...
| 6     | Content is JSON; each item also carries a plain-language `why`, ordered `how` steps and a per-item `reviewed` flag (added 2026-09-20); every item carries hadith or Qur'an evidence; a named reviewer signs off before release | "Evidence-backed" is the moat and is worthless unverified                                                                                                                                    |
...
```ts
const dir = Paths.appleSharedContainers['group.app.ihsaanly.companion']?.uri
...
tab does. What remains is about meaning, not ornament: no clock times (a
rough distance to the next prayer is allowed), no
