...
/** The Hijri date first, with the Gregorian date and the place beneath it. */
function HijriDateWidget(props: WidgetModel, environment: WidgetEnvironment): ReactElement {
  'widget'
  ...
  const url = 'ihsaanly://hijri'
  const date = props.date
  const detail = date.place ? `${date.gregorian} · ${date.place}` : date.gregorian

  if (family === 'accessoryInline') return inline(date.hijri, url)
  if (family === 'accessoryCircular') {
    // The day of the month leads the formatted date in every language's pattern we ship.
    const day = String(date.hijriDay)
    return circle(
      <VStack spacing={0}>
        <Image systemName="moon" size={11} />
        {say(day, 'title3', 'label', 1, 'bold')}
      </VStack>,
      url,
    )
  }
  if (family === 'accessoryRectangular') {
    return strip(
      <>
        {say(date.hijri, 'headline', 'label', 2)}
        {say(detail, 'caption', 'secondary', 1)}
      </>,
      url,
    )
  }
  if (family === 'systemSmall') {
    return panel(
      <>
        {heading(props.labels.hijri)}
        <Spacer />
        {say(date.hijri, 'title3', 'label', 3, 'bold')}
        {say(date.gregorian, 'footnote', 'secondary', 1)}
      </>,
      url,
    )
  }
  if (family === 'systemMedium') {
    return panel(
      <>
        {heading(props.labels.hijri)}
        <Spacer />
        {say(date.hijri, 'title', 'label', 2, 'bold')}
        {say(detail, 'subheadline', 'secondary', 1)}
      </>,
      url,
    )
  }
  return panel(
    <>
      {heading(props.labels.hijri)}
      <Spacer />
      {say(date.hijri, 'largeTitle', 'label', 3, 'bold')}
      {say(date.gregorian, 'title3', 'secondary', 1)}
      {date.place ? say(date.place, 'subheadline', 'secondary', 1) : null}
      <Spacer />
    </>,
    url,
  )
...
export default createWidget<WidgetModel>('HijriDateWidget', HijriDateWidget, sample)
