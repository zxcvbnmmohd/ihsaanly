...
/** The duas a phone cannot detect the moment for, one tap from the Home Screen. */
function QuickDuasWidget(props: WidgetModel, environment: WidgetEnvironment): ReactElement {
  'widget'
  ...
  const duas = props.quickDuas
  const first = duas[0] ?? null
  const url = first?.url ?? home

  if (family === 'accessoryInline') return inline(props.labels.quickDuas, url)
  if (family === 'accessoryCircular')
    return circle(<Image systemName="hands.sparkles" size={20} />, url)
  if (family === 'accessoryRectangular') {
    return strip(
      <>
        {say(props.labels.quickDuas, 'caption', 'secondary', 1, 'semibold')}
        {duas.slice(0, 2).map((entry) => (
  ...
  if (family === 'systemSmall') {
    return panel(
      <>
        {heading(props.labels.quickDuas)}
        {duas.slice(0, 3).map((entry) => row(entry, false))}
      </>,
      url,
    )
  }
  if (family === 'systemMedium') {
    // Two columns of two, each row its own tap target.
    const columns = [duas.slice(0, 2), duas.slice(2, 4)]
    return panel(
      <>
        {heading(props.labels.quickDuas)}
        <HStack alignment="top" spacing={12}>
          {(props.rtl ? [...columns].reverse() : columns).map((column, index) => (
            <VStack key={`column-${index}`} alignment={align} spacing={6}>
              {column.map((entry) => row(entry, true))}
            </VStack>
          ))}
        </HStack>
      </>,
  ...
  const limit = family === 'systemExtraLarge' ? 12 : 8
  return panel(
    <>
      {heading(props.labels.quickDuas)}
      {duas.slice(0, limit).map((entry) => row(entry, true))}
    </>,
    url,
  )
}

export default createWidget<WidgetModel>('QuickDuasWidget', QuickDuasWidget, sample)
